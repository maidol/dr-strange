...
// Eight arguments because this fork adds `embed_key_env` / `embed_model` on top
// of upstream's six. Grouping them would be a shared-signature change carried
// against upstream forever, for a function whose parameters are already the
// wire params one-for-one.
...
mod openrpc_param_drift {
    ...
    fn declared() -> Vec<(&'static str, Option<BTreeSet<String>>)> {
        vec![
            ...
            ("plane.vectorize", Some(wire_fields::<PlaneVectorize>())),
            ("plugin.list", None),
            ("plugin.catalog", None),
            ("plugin.install", Some(wire_fields::<PluginInstall>())),
            ("plugin.remove", Some(wire_fields::<PluginRemove>())),
        ]
    }
