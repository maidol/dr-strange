...
它放行的是**读取**。经 RPC 调用的 `plugin.install` 不读取该项，无论如何设置都只走
公网地址，因为插件是一个服务端随后会**执行**的 wasm 组件：放行内网 wiki 并不等于
同意运行由它提供的代码。若需从内网安装，请走 CLI——`drsg plugin install ./py.wasm`
接受本地路径，完全不经过网络，这也是下载被拒时的绕行办法。
